% function [rho, t, a] = calc_atmos(height, unit)
% Richard Rieber
% rrieber@gmail.com
% Updated 3/17/2006
%
...
%                 boolean, False for english units, True for metric
% Output o t - Temperature (K or R (default))
%        o p - Pressure (pa or psi (default))
%        o rho - Density (kg/m^3 or slug/ft^3 (default))
%        o a - speed of sound (m/s or ft/s (default))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = 1:length(height)
	...
	if unit
        t(j) = t(j)*K2R;
        p(j) = p(j)*pa2psi;
        rho(j) = rho(j)*kg2lbm/m2ft^3/32.174;
        a(j) = a(j)*m2ft;
	end
end
